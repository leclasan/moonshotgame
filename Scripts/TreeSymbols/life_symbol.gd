...
@export var modification = 1
@export var base_cost = 100
@export var max_upgrades = 2
@export var index = 1
@export var text = "[b]Extra Life[/b] [br] You have an extra life"
...
func _on_pressed() -> void:
	if WorldStats.money >= TreeStats.cost[index] and TreeStats.upgrades[index] < max_upgrades:
		WorldStats.player_max_life += modification 
		WorldStats.player_life += modification
